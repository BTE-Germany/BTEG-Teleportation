	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(sender instanceof ProxiedPlayer) {
			...
			if(args.length == 1) {
				...
				p.sendMessage(Main.getFormattedMessage("Bitte gib den Spieler an, der die Anfrage gesendet hat!"));
			}
		}
	}
		...
		if(sender instanceof ProxiedPlayer) {
			...
			if(player.hasPermission("teleportation.tpa")) {
				...
				if(!TpaCommand.tpas.containsValue(player.getUniqueId())) {
					player.sendMessage(Main.getFormattedMessage("Du hast keine Anfragen erhalten."));;
				}
			}
		}
		return results;
