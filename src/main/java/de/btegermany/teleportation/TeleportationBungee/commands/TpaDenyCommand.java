            ...
            if(args.length == 1) {
                ...
                if(target != null) {
                    if(TpaCommand.tpas.containsKey(target.getUniqueId()) && TpaCommand.tpas.get(target.getUniqueId()).equals(p.getUniqueId())) {
                        target.sendMessage(TeleportationBungee.getFormattedMessage("Deine Anfrage wurde abgelehnt!"));
                        p.sendMessage(TeleportationBungee.getFormattedMessage("Du hast die Anfrage von " + target.getName() + " abgelehnt."));
