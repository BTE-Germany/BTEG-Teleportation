...
public class ServersDetailGui extends MultiPageDetailWarpGuiAbstract {

    public ServersDetailGui(MultiPageGuiArgs guiArgs, String server) {
        super(guiArgs, server, Skulls.Skin.WARP_HOUSE.getId());
        ...
        inventory.addHandler(new PagedInventorySwitchPageHandler() {
            ...
            public void handle(SwitchHandler switchHandler) {
                ...
                pluginMessenger.send(new GetGuiDataMessage(registriesProvider, pluginMessenger, player.getUniqueId().toString(), "server_" + server, inventory.getSize()));
            }
        });
    }
