...
public class AllGui extends MultiplePagesDetailWarpGuiAbstract {

    public AllGui(Player player, PluginMessenger pluginMessenger, JSONArray contentJSON, RegistriesProvider registriesProvider) {
        super(player, "Alle Warps", pluginMessenger, contentJSON, Skulls.Skin.WARP_HOUSE.getId(), registriesProvider);
        ...
        inventory.addHandler(new PagedInventorySwitchPageHandler() {
            ...
            public void handle(SwitchHandler switchHandler) {
                ...
                pluginMessenger.send(new GetGuiDataMessage(registriesProvider, pluginMessenger, player.getUniqueId().toString(), "Alle", inventory.getSize()));
            }
        });
    }
